	...
	public static void displayAMain() throws SQLException {
		...
		if ((Boolean) result.get("isAdmin")) {
			...
			while(!number.equals("8")){
				...
				System.out.println("Press (4) to show orders by customerID. Press (5) to update order status      Press (6) to show all users and info");
				System.out.println("Press (7) to show all Orders            Press (8) to exit/Log Out");
				Scanner sc = new Scanner(System.in);
			...
			while(!input.equals("8")){
				...
				System.out.println("Press (6) to checkout from your current Shopping Cart. Press (7) to search for order.             Press (8) to exit to main menu./Log Out");
				System.out.print("Enter any number: ");
